...
func (c *Client) desktopBase(region string) string { return c.chatBase(region) }
...
func (c *Client) ReportDesktopEvent(a *auth.Auth, events ...DesktopEvent) error {
	...
	req, err := http.NewRequest(http.MethodPost, c.desktopBase(snap.Region)+desktopReportPath, bytes.NewReader(raw))
	if err != nil {
		return err
	}
	...
	req.Header.Set("X-Domain", c.desktopBase(a.Snapshot().Region))
...
func (c *Client) SetAppearanceTheme(a *auth.Auth, resourceKey string) error {
	...
	snap := a.Snapshot()
	req, err := http.NewRequest(http.MethodPost, c.desktopBase(snap.Region)+desktopAppearanceSet, bytes.NewReader(raw))
	if err != nil {
		return err
	}
	req.Header.Set("Authorization", "Bearer "+snap.AccessToken)
...
func (c *Client) ReportWebEvent(a *auth.Auth, eventCode, pageURL, elementID, elementName string) error {
	...
	ev := map[string]any{
		...
		"machineId": deriveID(a, "webmachine"), "userId": snap.UID,
		"userNickname": snap.Nickname, "enterpriseId": snap.EnterpriseID,
	}
...
func (c *Client) MarketExpertList(a *auth.Auth, expertType string) ([]MarketExpert, error) {
	...
	snap := a.Snapshot()
	req, err := http.NewRequest(http.MethodPost, c.chatBase(snap.Region)+"/portal/operation-platform/market/expert/list", bytes.NewReader(raw))
	if err != nil {
		return nil, err
	}
	req.Header.Set("Authorization", "Bearer "+snap.AccessToken)
	req.Header.Set("Content-Type", "application/json")
	req.Header.Set("User-Agent", desktopUA)
	req.Header.Set("X-Domain", c.chatBase(snap.Region))
	req.Header.Set("X-Product", "SaaS")
	if snap.UID != "" {
		req.Header.Set("X-User-Id", snap.UID)
	}
...
func (c *Client) DesktopChatWithExpert(a *auth.Auth, expertID string) (conversationID, requestID string, err error) {
	...
	snap := a.Snapshot()
	req, err := http.NewRequest(http.MethodPost, c.chatBase(snap.Region)+"/v2/chat/completions", bytes.NewReader(raw))
	if err != nil {
		return "", "", err
	}
	h := req.Header
	...
	h.Set("X-Domain", c.chatBase(snap.Region))
	h.Set("X-Product", "SaaS")
	h.Set("X-User-Id", snap.UID)
